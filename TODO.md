...
- [x] **Analytics por projeto + aba "Impacto Positivo" + transparência pública** — cada projeto (`financial_projects`) ganhou um Analytics próprio (gráficos recharts de custos: por categoria, fixo×variável, evolução mensal), opção de **puxar dados da Capacitação por programa** (PROINDI 4.0/PTI), e uma nova tabela `project_impacts` (números de impacto flexíveis: rótulo+valor+texto, editáveis). Toggles de visibilidade em **2 níveis** (master global do portal via `app_settings` + master por projeto `impactoPublico`) + `isPublic` por item + `analyticsPublico` pros gráficos/Capacitação — tudo sob o gate `visivelTransparencia`. Componente reutilizável `project-analytics-modal.tsx` (admin com dados completos + botão olho; público filtrado). Nova aba "Impacto Positivo" no admin; na `/transparencia` um CTA/olho abre o modal. Rotas admin (analytics/impactos/settings) + pública `/api/public/projetos/:id/analytics`. Migração aplicada no Neon (mesmo banco de prod). **Verificado E2E no backend:** custos reconciliam (fixos+variáveis=despesas), Capacitação vinculada bate (219 matrículas PROINDI), item privado NÃO vaza no público, master global OFF zera o impacto, projeto não-transparente → 404. *Render visual dos gráficos a conferir no navegador (Playwright fora no momento).* *2026-07-16*
