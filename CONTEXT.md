...
- **Programa do curso (PROINDI 4.0 × PTI):** coluna `courses.program` (`text NOT NULL DEFAULT 'pti'`; valores `'proindi' | 'pti'`). Constantes em `shared/schema.ts`: `COURSE_PROGRAMS` + `COURSE_PROGRAM_LABELS` (`proindi` → **"PROINDI 4.0"**, `pti` → **"PTI"**) — o rótulo oficial é "PROINDI 4.0", não "PRONDI"/"PROINDO". O default `'pti'` faz o backfill: cursos antigos viram PTI e só os do PROINDI são marcados. **Os 6 do PROINDI 4.0:** Aplicação de IA's · Processos avaliativos da maturidade da indústria 4.0 · Transformação Digital · Inovação Tecnológica na Indústria · Logística e Cadeia de Suprimentos · Lean Manufacturing. Os demais (Fundamentos & Gestão, Maturidade 4.0 & Projeto Final, Sustentabilidade e ESG) são PTI. No painel: seletor **Programa** no form do curso, **selo** no card (indigo=PROINDI, âmbar=PTI) e **filtro por programa** com contagem (Todos/PROINDI 4.0/PTI) — combina com o filtro de status. `insertCourseSchema` tem `program` **opcional** (quem não informa cai no default da coluna — o seed de cursos depende disso)
- **Analytics por projeto + Impacto Positivo (projetos-admin ↔ /transparencia):** cada projeto (`financial_projects`) tem um **Analytics** próprio via `storage.getProjectAnalytics(id, {publicOnly?})`: custos (receitas/despesas/saldo, **despesas por categoria**, **fixo × variável** por `tipoCusto`, **evolução mensal** por mês de `data`) agregados das `financial_transactions` do projeto; **Capacitação** (se `financial_projects.programaCapacitacao` = 'proindi'|'pti' → reusa `getCapacitacaoAnalytics(program)`); e **Impacto Positivo** (tabela nova `project_impacts`: `titulo`/`valor`(flexível)/`descricao`/`isPublic`/`ordem`, FK cascade). Componente reutilizável `client/src/components/project-analytics-modal.tsx` (recharts — 1º gráfico real do app) renderiza tudo no admin (dados completos, botão **olho** "Análise" no card) e no público (dados já filtrados). **Toggles de visibilidade — 2 chaves (simplificado 2026-07-20):** master GLOBAL do portal (`app_settings` key `impacto_positivo_publico`) + `isPublic` por item. Um número aparece no público sse **global ON && item.isPublic**. O gate antigo por projeto `impactoPublico` foi **aposentado** (coluna mantida no schema, mas não é mais lida por nenhum endpoint — evitava a confusão de "liguei o item e não aparece" por causa de flags esquecidos, seguindo a doutrina "toggle acende o pacote"). O interruptor mestre aparece em 2 lugares (card no topo da aba "Projetos em Execução" **e** dentro da sub-aba "Impacto Positivo" do formulário, com aviso âmbar quando desligado) — mesma query/mutation. O analytics detalhado (gráficos+Capacitação) segue gated por `analyticsPublico`; tudo respeita o gate existente `visivelTransparencia`. **Rotas:** admin `GET /api/admin/financeiro/projetos/:id/analytics`, CRUD `/api/admin/projetos/:id/impactos`, `GET/PATCH /api/admin/settings/impacto-publico`; público `GET /api/public/projetos/:id/analytics` (404 se não transparente; impacto só se global+projeto+item ligados; Capacitação só se analyticsPublico). `/api/public/transparencia` ganhou, por projeto: `temAnalytics` (mostra o CTA/olho), **`impactos[]`** (só os itens públicos, gated por global+`impactoPublico`) e **`impactoIntro`** — e a página pública `transparencia.tsx` renderiza esses números **inline** numa seção "Impacto Positivo" (cards com o `valor` cru — número **ou** porcentagem, ex. "1.200" / "85%"), além do olho que abre o modal completo. A gestão do impacto fica numa **sub-aba "Impacto Positivo" DENTRO do formulário de cada projeto** (ao lado de "Doações"): ali o gestor marca o vínculo **opcional** com a Capacitação (PROINDI 4.0/PTI — independente dos números próprios), liga a seção pública do projeto (`impactoPublico`, salvo no submit), edita o texto de abertura e **adiciona/remove livremente quantos números de impacto quiser** (botão **"+ Adicionar rótulo"**; CRUD keyed em `editingProject.id`; projeto novo pede pra salvar antes de adicionar itens). O **master global do portal** fica num card no topo da aba "Projetos em Execução". Migração aplicada direto no Neon (mesmo banco de prod)
- **Sub-aba Analytics da Capacitação:** `GET /api/capacitacao/analytics` (`requireAdmin`) → `storage.getCapacitacaoAnalytics()`. Métricas: certificados emitidos, **alunos formados = pessoas distintas por CPF** (fallback e-mail/nome) entre os certificados, cursos concluídos, total de matrículas, matrículas com × sem empresa, ranking de cursos por matrícula (**com o programa de cada curso**), ranking de empresas de origem, e **`resumoProgramas`** (totais por programa: cursos, concluídos, matrículas, certificados — PROINDI 4.0 sempre antes do PTI). O relatório em PDF traz o bloco **"Resumo por programa"** + a coluna **"Programa"** no ranking de cursos. Componente `CapacitacaoAnalyticsTab`
- **Relatórios exclusivos por programa:** `GET /api/capacitacao/analytics?program=proindi|pti` refaz TODAS as métricas **escopadas ao programa** (inválido/ausente = geral). Como `enrollments`/`certificates` **não têm coluna de programa** (ele está só em `courses.program`), o `getCapacitacaoAnalytics(program?)` filtra os `cursos` do programa, deriva o set de `courseIds` e **guarda os laços de inscrições/formados/empresas + o `totalMatriculas` por `courseId`** (junta via curso). No painel, **abaixo de cada card do "Resumo por programa"** há um botão **"Baixar relatório PROINDI 4.0" / "Baixar relatório PTI"** que faz o fetch escopado e chama `baixarRelatorioAnalytics(scoped, program)`. O relatório de um programa **omite** o bloco "Resumo por programa" e a coluna "Programa" (redundantes) e o título/nome do arquivo identificam o programa. Estado `baixandoRel` (`'geral'|'proindi'|'pti'`) garante que só o botão clicado mostra "Gerando…". Reconciliação garantida por construção (todo curso pertence a 1 programa e toda inscrição a 1 curso → PROINDI + PTI = geral; `alunosFormados` é distinto por CPF, então não soma — cada um ≤ geral) (4ª aba do dashboard, espelha o visual do Analytics de Marketing)
