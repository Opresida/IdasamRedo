...
- **Botão "Disparar Certificados"** (dashboard) só aparece enquanto houver inscrito sem certificado. Quando `certifiedCount >= enrolledCount` (todos já emitidos), vira o selo "Certificados enviados". `certifiedCount` é **derivado** (não é coluna): conta certificados com `fileData` por curso em `getCourses()`/`getCourseWithCount()`, mesma regra do badge "Enviado" da lista de alunos. Mutações de aluno/certificado invalidam `['/api/courses']` para manter o contador fresco
