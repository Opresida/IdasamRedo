...
- **Campo "Empresa" na inscrição:** `enrollments.company` (nullable) — opcional, capta se o inscrito vem de uma empresa (filtro/analytics + porta de entrada PD&i). Presente em todos os cadastros: modal público (`enrollment-dialog.tsx`, com tooltip de ajuda), form admin (`EnrollmentFormDialog`), import/export CSV (coluna `empresa`) e coluna na lista de alunos. `insertEnrollmentSchema` (createInsertSchema sem extend) já inclui `company` automaticamente
...
- **Lista de chamada (presença) por curso:** botão no dashboard (`CourseEnrollments`) gera a lista de presença do curso com os alunos cadastrados, no modelo fixo (logo IDASAM, metadados do curso, tabela Nº/Nome/CPF/Assinatura, rodapé fixo `www.idasam.org` com `tfoot` espaçador que evita cortes). Util `client/src/lib/lista-chamada.ts` (`buildListaChamadaHtml` + `printListaChamada`) monta o HTML e imprime via **iframe oculto** (`window.print` → Salvar como PDF, fidelidade vetorial idêntica ao modelo). Alunos ordenados A-Z; CPF formatado (`XXX.XXX.XXX-XX` quando 11 dígitos). Logo SVG inline em `client/src/lib/idasam-logo-chamada-svg.ts` (módulo TS, **não** usar import `?raw` — trava o dev server no boot)
