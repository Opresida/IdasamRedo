/**
 * IDASAM Suite Documental
 * Coloque em: client/src/components/suite-documental/SuiteDocumental.tsx
 *
 * Dependências a instalar (rode no terminal do Replit):
...
import {
  ...
  Download, ArrowLeft, Plus, X, Loader2,
} from 'lucide-react'
import './suite.css'
...
// CONSTANTES
// ════════════════════════════════════════════════════════
const LOGO = 'https://raw.githubusercontent.com/Opresida/IdasamRedo/refs/heads/main/logo.svg'
// Altura útil por página = 1123 - stripe(5) - header(110) - stripe(3) - footer(44) - padding(88) - margem(50)
const MAX_H = 823
...
// ════════════════════════════════════════════════════════
// TEMPLATE DA PÁGINA A4 (DOM puro — necessário para html2canvas)
// ════════════════════════════════════════════════════════
...
async function autoPaginate(
  ...
  container.innerHTML = ''
  let pageCount = 1
  let currentPage = createA4Page(docType, isOrc)
  container.appendChild(currentPage)
  let contentBox = currentPage.querySelector('.sd-content') as HTMLElement

  for (const el of Array.from(source.children)) {
    const clone = el.cloneNode(true) as HTMLElement
    contentBox.appendChild(clone)
    await new Promise(r => requestAnimationFrame(r))
    if (contentBox.scrollHeight > MAX_H) {
      contentBox.removeChild(clone)
      pageCount++
      currentPage = createA4Page(docType, isOrc)
      container.appendChild(currentPage)
      contentBox = currentPage.querySelector('.sd-content') as HTMLElement
      contentBox.appendChild(clone)
    }
  }

  container.querySelectorAll('.sd-page-num').forEach((el, i) => {
...
export function SuiteDocumental() {
  ...
  // ════════════════════════════════════════════════════
  // CONSTRUTORES DE HTML DOS DOCUMENTOS
  // ════════════════════════════════════════════════════
  ...
  function buildOrcHTML(): string {
    ...
    if (oData.acrescimo > 0) h += `<div class="sd-total-line"><span>Acréscimo (${oData.acrescimo}%):</span><span style="color:#1A4D2E">+ ${fmtBRL(acrVal)}</span></div>`
  ...
  async function handlePreview(tipo: PreviewType) {
    setPreview(tipo)
    setPaginating(true)
    // Aguarda o DOM montar os refs dentro do overlay
  ...
  // ════════════════════════════════════════════════════
  // HELPERS DE ESTADO — CLÁUSULAS / ASSINATURAS / ITENS
  // ════════════════════════════════════════════════════
  ...
  return (
    <div className="relative">
      ...
      {/* ── Loading PDF ── */}
      {generatingPDF && (
        <div className="fixed inset-0 z-[9999] bg-[#0D2318]/92 flex flex-col items-center justify-center gap-5">
        ...
        <div className="sd-preview-wrap">
          ...
          {paginating && (
            <div className="flex flex-col items-center justify-center gap-3 mt-20">
              <Loader2 className="h-8 w-8 text-[#1A4D2E] animate-spin" />
              <p className="text-[#0D2318] font-semibold text-sm">Montando documento...</p>
            </div>
          )}
      ...
      {/* ════════════════════════════════════════════
          EDITOR PRINCIPAL
      ════════════════════════════════════════════ */}
      <div className="min-h-screen bg-[#EDF3EE]">

        {/* Header institucional */}
        <div className="bg-[#0D2318] px-8 py-5 flex items-center gap-4 border-b border-[#FBBF24]/20 shadow-lg">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <Tabs defaultValue="contratos" className="w-full">
            ...
            <TabsList className="grid grid-cols-3 w-full h-12 mb-8 bg-[#0D2318]/8 border border-[#C8DDD5]">
              <TabsTrigger value="contratos"  className="gap-2 data-[state=active]:bg-[#1A4D2E] data-[state=active]:text-white">
            ...
            <TabsContent value="contratos" className="space-y-4">
              ...
              <Card title="Identificação do Documento">
                <div className="grid grid-cols-3 gap-4">
                  <Field label="Cidade e Data">
              ...
              <Card title="Cláusulas do Contrato">
                <div className="flex gap-1 bg-[#EEF5F0] p-1 rounded-lg mb-5">
                  {(['estruturado', 'livre'] as const).map(m => (
                    <button key={m} onClick={() => setCMode(m)}
                      className={`flex-1 py-2 rounded-md text-sm font-semibold transition-all ${cMode === m ? 'bg-white text-[#0D2318] shadow' : 'text-[#4A7260]'}`}>
                ...
                {cMode === 'estruturado' ? (
                  <div className="flex flex-col gap-3">
                    {clauses.map((c, i) => (
                      <div key={c.id} className="border border-[#C8DDD5] rounded-xl overflow-hidden bg-[#FAFCFB]">
                        <div className="flex items-center gap-3 px-4 py-2 bg-[#EEF5F0] border-b border-[#C8DDD5]">
                          <div className="w-6 h-6 rounded-full bg-[#0D2318] text-white flex items-center justify-center text-[11px] font-bold flex-shrink-0">{i + 1}</div>
                          <input className="flex-1 bg-transparent border-none text-[11px] font-bold uppercase tracking-wide text-[#0D2318] outline-none placeholder:text-gray-400"
                  ...
                  <Field label="Cole ou escreva a minuta completa">
                ...
                <div className="flex flex-col gap-3">
                  {sigs.map(s => (
                    <div key={s.id} className="border border-[#C8DDD5] rounded-xl overflow-hidden bg-[#FAFCFB]">
                      <div className="flex items-center gap-3 px-4 py-2 bg-[#EEF5F0] border-b border-[#C8DDD5]">
                        <div className="w-6 h-6 rounded-full bg-[#0D2318] text-white flex items-center justify-center text-xs flex-shrink-0">✍</div>
                        <input className="flex-1 bg-transparent border-none text-[11px] font-bold uppercase tracking-wide text-[#0D2318] outline-none placeholder:text-gray-400"
              ...
              <div className="flex justify-end gap-3 pt-1">
                ...
                <Button onClick={() => handlePreview('contratos')} className="bg-[#1A4D2E] hover:bg-[#2A5B46] text-white gap-2">
            ...
            <TabsContent value="orcamentos" className="space-y-4">
              ...
              <Card title="Identificação do Orçamento">
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <Field label="Título / Objeto" className="col-span-4">
              ...
              <Card title="Itens do Orçamento">
                ...
                {oItems.map(it => (
                  <div key={it.id} className="grid grid-cols-[2fr_80px_130px_130px_36px] gap-2 items-center py-2 border-b border-gray-100">
                    ...
                    <div className="text-right font-bold text-sm text-[#0D2318]">{fmtBRL(it.qty * it.unit)}</div>
                ...
                <div className="mt-4 bg-[#EEF5F0] rounded-xl p-4 flex flex-col items-end gap-2 border border-[#C8DDD5]">
                  ...
                  {oData.acrescimo > 0 && <TotalRow label={`Acréscimo (${oData.acrescimo}%)`} value={`+ ${fmtBRL(acrVal)}`}  color="text-[#1A4D2E]" />}
                  <div className="flex gap-6 text-base font-bold text-[#0D2318] border-t border-[#C8DDD5] pt-2 mt-1 w-full justify-end">
                    <span className="w-36 text-right">TOTAL GERAL:</span>
                    <span className="w-32 text-right text-[#1A4D2E]">{fmtBRL(totalOrc)}</span>
                  </div>
                </div>
              </Card>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {[
                    ...
                    <button key={p.val} onClick={() => setOData(d => ({ ...d, pagamento: p.val }))}
                      className={`flex flex-col gap-1 p-3 rounded-xl border-2 text-left transition-all ${oData.pagamento === p.val ? 'border-[#1A4D2E] bg-[#EEF5F0]' : 'border-[#C8DDD5] bg-[#FAFCFB] hover:border-[#4E8D7C]'}`}>
                      <span className="text-sm font-semibold text-[#0D2318]">{p.label}</span>
                      <span className="text-xs text-[#4A7260]">{p.desc}</span>
                    </button>
                  ))}
                </div>
                {oData.pagamento === 'personalizado' && (
                  <Field label="Condição personalizada" className="mb-4">
              ...
              <Card title="Responsável pela Proposta">
                <div className="grid grid-cols-3 gap-4">
                  <Field label="Nome">
            ...
            <TabsContent value="oficios" className="space-y-4">
              ...
              <Card title="Identificação do Emissor">
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Nome da Empresa / Órgão">
              ...
              <Card title="Destinatário">
                <div className="grid grid-cols-3 gap-4">
                  <Field label="Nome Completo" className="col-span-2">
                    <Input value={ofData.destNome} onChange={e => setOfData(d => ({ ...d, destNome: e.target.value }))} placeholder="Ex: João da Silva" />
                  </Field>
                  <Field label="Tratamento">
                    <Select value={ofData.destTratamento} onValueChange={v => setOfData(d => ({ ...d, destTratamento: v }))}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {['Ao Sr.','À Sra.','À Exma. Sra.','Ao Exmo. Sr.','À Ilma. Sra.','Ao Ilmo. Sr.','À Instituição'].map(t =>
                  ...
                  <Field label="Empresa / Órgão / Instituição" className="col-span-2">
              ...
              <Card title="Assunto">
                <Field label="Resumo Objetivo do Ofício">
              ...
              <Card title="Corpo do Texto">
                <div className="flex flex-col gap-4">
                  <Field label="➤ Introdução — Apresentação e motivo do ofício">
                  ...
                  <Field label="➤ Desenvolvimento — Detalhamento do pedido ou posicionamento">
                  ...
                  <Field label="➤ Conclusão — O que se espera (ação, resposta, prazo)">
              ...
              <Card title="Fundamentação Jurídica / Normativa (opcional)">
                <Field label="Leis, contratos, normas ou regulamentos aplicáveis">
              ...
              <Card title="Assinatura">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <Field label="Instituição (linha da assinatura)" className="col-span-3">
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#C8DDD5] p-7 shadow-sm">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-1 h-4 rounded-sm bg-gradient-to-b from-[#1A4D2E] to-[#008080] flex-shrink-0" />
        <h3 className="text-[11px] font-bold uppercase tracking-[0.1em] text-[#0D2318]">{title}</h3>
      </div>
      {children}
    </div>
  )
...
function DashedAdd({ onClick, label }: { onClick: () => void; label: string }) {
  return (
    <button onClick={onClick}
      className="flex items-center gap-2 w-full mt-2 border-2 border-dashed border-gray-300 hover:border-[#1A4D2E] rounded-xl py-3 px-4 text-sm font-semibold text-gray-500 hover:text-[#1A4D2E] hover:bg-[#EEF5F0] transition-all">
      <Plus size={16} /> {label}
    </button>
  )
}
